...
async function sendBalanceUpdate(balance: any) {
  try {
    await axios.post(
      'https://leonos-dashboard.onrender.com/api/update-balance',
      balance,
      {
        auth: {
    ...
    console.log("✅ Sent balance update to dashboard");
  } catch (err: any) {
    console.error("❌ Failed to send balance update:", err.message);
  }
}

async function sendTradeLog(logEntry: any) {
  try {
    await axios.post(
      'https://leonos-dashboard.onrender.com/api/update-log',
      logEntry,
      {
        auth: {
          username: process.env.DASHBOARD_USER!,
    ...
    console.log("✅ Trade log sent to dashboard");
  } catch (err: any) {
    console.error("❌ Failed to send trade log:", err.message);
  }
}
...
function calculateBuyScore({ rsiValue, rsiTrendUp, macdCrossUp, priceAboveSMA20, priceAboveSMA50, volumeSpike }) {
  let score = 0;
  if (rsiValue < 30 && rsiTrendUp) score += 30;
  if (macdCrossUp) score += 30;
  if (priceAboveSMA20 && priceAboveSMA50) score += 20;
...
async function run() {
  ...
  while (true) {
    try {
      ...
      await sendBalanceUpdate(nonZeroBalances);


      for (const SYMBOL of symbols) {

        if (blacklistedSymbols.includes(SYMBOL)) {
          logMessage(`Skipping blacklisted symbol: ${SYMBOL}`, SYMBOL, 'INFO');
          continue;
        }
  ...
  macdCrossUp: macd[macd.length - 1]?.MACD > macd[macd.length - 1]?.signal &&
...
logMessage(`Score: ${score} (No buy. Waiting for score ≥ 80)`, SYMBOL, 'INFO');

if (!tradeData[SYMBOL].buyPrice && score >= 80) {
